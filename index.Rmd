...
- [Initial Visualizations](#initial-visualisations)
...
My next visualization aimed to be more interactive. 
This time, I managed to get individual interactive visualizations for each year. However, this did not allow us to visualize the evolution across time in a single plot. 
...
# Create interactive visualizations for each year
...
```{r}
# Enhanced plot with light yellow to dark red gradient and updated hover text
enhanced_plot <- plot_ly(
  ...
  text = ~ifelse(
    is.na(Prevalence),
    NA,  # No hover text for missing values
  ...
  zmin = 0,
  zmax = 70,  # Adjusted max prevalence for better differentiation
  showscale = TRUE,
  ...
  layout(
    ...
    annotations = list(
      ...
      list(
        ...
        y = -0.1,                                  # Vertical position (below the map)
        xref = "paper",                            # Relative to paper's x-axis
        yref = "paper",                            # Relative to paper's y-axis
        text = "Note: White regions indicate missing data.", # Annotation text
        showarrow = FALSE,                         # No arrow
        font = list(size = 12, color = "black"),   # Font styling
        align = "left"
      ),
      # Source annotation
      list(
        x = 0.5,                                   # Horizontal position
        y = -0.15,                                 # Vertical position (below the map)
        xref = "paper",                            # Relative to paper's x-axis
        yref = "paper",                            # Relative to paper's y-axis
        text = "Source: Multiple sources compiled by World Bank (2024) – processed by Our World In Data",
        showarrow = FALSE,                         # No arrow
        font = list(size = 10, color = "black"),   # Font styling
        align = "center"
      )
    )
  )
